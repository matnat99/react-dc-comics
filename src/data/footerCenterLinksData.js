const footerCenterLinks = [
  {
    id: 1,
    lists: [
      {
        id: 1,
        title: "DC COMICS",
        links: [
          { id: 1, text: "Characters", url: "#" },
          { id: 2, text: "Comics", url: "#" },
          { id: 3, text: "Movies", url: "#" },
          { id: 4, text: "TV", url: "#" },
          { id: 5, text: "Games", url: "#" },
          { id: 6, text: "Videos", url: "#" },
          { id: 7, text: "News", url: "#" },
        ],
      },
      {
        id: 2,
        title: "SHOP",
        links: [
          { id: 8, text: "Shop DC", url: "#" },
          { id: 9, text: "Shop DC Collectibles", url: "#" },
        ],
      },
    ],
  },
  {
    id: 2,
    lists: [
      {
        id: 1,
        title: "DC",
        links: [
          { id: 1, text: "Terms of Use", url: "#" },
          { id: 2, text: "Privacy Policy (New)", url: "#" },
          { id: 3, text: "Add Choices", url: "#" },
          { id: 4, text: "Advertising", url: "#" },
          { id: 5, text: "Jobs", url: "#" },
          { id: 6, text: "Subscriptions", url: "#" },
          { id: 7, text: "Talent Workshops", url: "#" },
          { id: 8, text: "CPSC Certificates", url: "#" },
          { id: 9, text: "Ratings", url: "#" },
          { id: 10, text: "Shop Help", url: "#" },
          { id: 11, text: "Contact Us", url: "#" },
        ],
      },
    ],
  },
  {
    id: 3,
    lists: [
      {
        id: 1,
        title: "SITES",
        links: [
          { id: 1, text: "DC", url: "#" },
          { id: 2, text: "MAD Magazine", url: "#" },
          { id: 3, text: "DC Kids", url: "#" },
          { id: 4, text: "DC Universe", url: "#" },
          { id: 5, text: "DC Power Visa", url: "#" },
        ],
      },
    ],
  },
];

export default footerCenterLinks;
